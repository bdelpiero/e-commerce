    ...
    .then((order) => {
      ...
      return product.then((product) => {
       return product.update({stock: product.stock - quantity})
        .then(()=> Order_Product.findOrCreate({where: {productId:product.id, orderId: order[0].id}}))
        .then(orderProduct => orderProduct[0].update({total: orderProduct[0].total + 1}))
...
router.delete("/user/:productId", (req, res, next)=>{
  const { productId } = req.params;
  ...
  Order.findOne({where: {userId: req.user.id , status: "Pendiente"}})
    .then(order => {
      console.log(order)
      Order_Product.findOne({where:{orderId: order.id, productId: productId}})
        .then((orderProduct)=> Product.findByPk(productId)
          .then(product => product.update({stock: orderProduct.total + product.stock})))
      return Order_Product.destroy({where:{orderId: order.id, productId: productId}})
    })
...
router.delete("/:orderId" , (req, res, next)=>{
  Order.destroy({where:{userId: req.user.id, status: "Pendiente"}})
  .then(()=> res.sendStatus(200))
})
